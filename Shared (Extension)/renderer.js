...
window.JSONRenderer = (() => {
  ...
  function render(parsed, indentSize) {
    const INDENT_PX = typeof indentSize === 'number' ? indentSize * 7 : 14;
    const dateNodes = [];
    let lineCount = 0;
    ...
    function renderDateValue(origVal) {
      ...
      el.addEventListener('click', () => {
        ...
        dateNodes.forEach(node => {
          ...
          node.el.textContent = (newFmt === 0 && isN) ? val : `"${val}"`;
    ...
    function renderValue(val, depth, trailingComma, keyPrefix) {
      ...
      if (Array.isArray(val)) { renderCollection(val, depth, trailingComma, keyPrefix, true); return; }
      if (typeof val === 'object') { renderCollection(val, depth, trailingComma, keyPrefix, false); return; }
    }
    ...
    function renderCollection(val, depth, trailingComma, keyPrefix, isArray) {
      ...
      if (isArray) {
        val.forEach((item, i) => renderValue(item, depth+1, i < count-1, null));
      } else {
        keys.forEach((k, i) => {
          renderValue(val[k], depth+1, i < count-1, [p('jv-key',`"${esc(k)}"`), p('jv-punct',': ')]);
        });
      }

      // Snapshot element references now, before closeLine is appended
      const bodyLines = Array.from(container.children).slice(bodyStart);
      ...
      let collapsed = false;
      function collapse() {
        collapsed = true; toggle.classList.add('collapsed');
        bodyLines.forEach(l => l.style.display = 'none');
        closeLine.style.display = 'none'; summary.style.display = '';
      }
      function expand() {
        collapsed = false; toggle.classList.remove('collapsed');
        bodyLines.forEach(l => l.style.display = '');
        closeLine.style.display = ''; summary.style.display = 'none';
      }
      toggle.addEventListener('click', e => { e.stopPropagation(); collapsed ? expand() : collapse(); });
      summary.addEventListener('click', e => { e.stopPropagation(); expand(); });
    }
  ...
  function getPlainText() { return null; }

  return { render, getPlainText, FMT_LABELS };
})();
