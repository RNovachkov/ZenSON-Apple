(() => {
  ...
  function buildTree(parsed, doc) {
    ...
    function mkLine(depth){const l=doc.createElement('span');l.className='jv-line';lineCount++;const sp=doc.createElement('span');sp.style.cssText='display:inline-block;width:14px';l.appendChild(sp);l.appendChild(mkIndent(depth));return l}
    ...
    function renderColl(val,depth,comma,keyPfx,isArr){
      ...
      openLine.replaceChild(toggle,openLine.firstChild);
      ...
      let collapsed=false;
      function doCollapse(){collapsed=true;toggle.classList.add('collapsed');bodyLines.forEach(l=>l.style.display='none');closeLine.style.display='none';summary.style.display=''}
      function doExpand(){collapsed=false;toggle.classList.remove('collapsed');bodyLines.forEach(l=>l.style.display='');closeLine.style.display='';summary.style.display='none'}

      toggle.addEventListener('click',e=>{e.stopPropagation();collapsed?doExpand():doCollapse();});
      summary.addEventListener('click',e=>{e.stopPropagation();doExpand();});
    }
